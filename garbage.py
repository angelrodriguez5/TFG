...
plt.rcParams['animation.ffmpeg_path'] = r'C:\Users\pickl\Downloads\ffmpeg-20191126-59d264b-win64-static\bin\ffmpeg.exe'
# Set up formatting for the movie files
writer = animation.FFMpegWriter(fps=5, metadata=dict(artist='Me'), bitrate=1800)

fig3, (ax_i, ax_g)= plt.subplots(1,2,figsize=[20,8])
...
for batch_i, (frame_secs, input_imgs) in enumerate(tqdm.tqdm(dataloader, desc="Analysing video")):
    ...
    imgs.extend([img.permute(1, 2, 0).numpy() for img in input_imgs])
    bboxes.extend(outputs)
...
im = ax_i.imshow(imgs[0])
gr, = ax_g.plot([],[])
ax_g.set_xlim(-5, 100)
ax_g.set_ylim(-100, 20000)

def update_bbox(idx, imgs, bboxes):
    global ax_i, ax_g, im, gr, frames, total_area

    # Update image
    # Clean previous iteration patches
    [p.remove() for p in ax_i.patches]
    # Set new image
    im.set_data(imgs[idx])
    # Print bboxes
    if bboxes[idx] is not None:
        for *coords, conf, cls_conf, cls_pred in bboxes[idx]:
            x1, y1, x2, y2 = [int(x) for x in coords]
            w = x2-x1
            h = y2-y1
            ax_i.add_patch(Rectangle((x1,y1),w,h, linewidth=1, edgecolor='b', facecolor="none"))

    # Update graph
    gr.set_xdata(frames[:idx])
    gr.set_ydata(total_area[:idx])
...
line_ani = animation.FuncAnimation(fig3, update_bbox, len(imgs), fargs=(imgs, bboxes),
                                   interval=50, blit=True)
line_ani.save('lines.mpeg', writer=writer)
