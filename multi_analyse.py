...
def print_expert_timings(ax, videoName, framerate, expert=None):
    ...
    for stage, color in zip(list(video.values()), colors):
        if expert is not None:
            ...
            # print mean
            # ax.axvline(x=mean(stage), color=color)
            # Print all values
            for value in stage:
                ax.axvline(x=value, color=color)
...
if __name__ == "__main__":
    ...
    videos = [('C:\\Users\\pickl\\Documents\\UDC2018\\TFG-NoGit\\HET-CAM\\Controles_Negativos_y_Positivos\\DSC_1089.MOV', 'r'),
              ('C:\\Users\\pickl\\Documents\\UDC2018\\TFG-NoGit\\HET-CAM\\Controles_Negativos_y_Positivos\\DSC_1107.MOV', 'r'),
              ('C:\\Users\\pickl\\Documents\\UDC2018\\TFG-NoGit\\HET-CAM\\Controles_Negativos_y_Positivos\\DSC_1103.MOV', 'b'),
              ('C:\\Users\\pickl\\Documents\\UDC2018\\TFG-NoGit\\HET-CAM\\Controles_Negativos_y_Positivos\\DSC_1106.MOV', 'b')]

    fig, (ax1,ax2) = plt.subplots(2, 1, sharex=True)
    ax1.set_title("Area of bleeding")
    ax2.set_title("Total number of detections")

    for video_path, color in videos:
        ...
        for batch_i, (frame_times, input_imgs) in enumerate(tqdm.tqdm(dataloader, desc="Analysing video")):
            # Configure input
        ...
        # Ended analysing video, print results in appropriate plot
        ax1.plot(frames, total_area, color=color, label=videoName)
        ax2.plot(frames, num_of_detections, color=color)
